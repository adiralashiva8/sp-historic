...
def compare(db):
    if request.method == "POST":
        ...
        cursor.execute("SELECT Table_Name, Execution_Id, Response_Time, Sql_Count, Sql_Time from TB_TEST WHERE Execution_Id=%s;" % eid_one )
        first_data = cursor.fetchall()
        # fetch second eid test results
        cursor.execute("SELECT Table_Name, Execution_Id, Response_Time, Sql_Count, Sql_Time from TB_TEST WHERE Execution_Id=%s;" % eid_two )
