...
def sphistoric_setup(opts):
    ...
    print("INFO: Creating sphistoric dB")
    try:
        obj.execute("CREATE DATABASE IF NOT EXISTS sphistoric;")
    except Exception as e:
        print(str(e))

    print("INFO: Creating TB_PROJECT table")
    rfdb = connect_to_mysql_db(opts.host, opts.username, opts.password, "sphistoric")
    try:
        rfobj = rfdb.cursor()
        rfobj.execute("CREATE TABLE IF NOT EXISTS TB_PROJECT ( Project_Id INT NOT NULL auto_increment primary key, Project_Name TEXT, Project_Desc TEXT, Project_Image TEXT, Created_Date DATETIME, Last_Updated DATETIME, Total_Executions INT);")
    except Exception as e:
        print(str(e))
